...
export const useStore = create(devtools((set, get) => ({
  ...
  fetchStaff: async () => {
    ...
    try {
      const data = await apiCall("/auth/staff");
      set({ staff: Array.isArray(data?.members) ? data.members : [] });
    } catch (e) {
  ...
  addStaffMember: async (data) => {
    const saved = await apiCall("/auth/staff", {
      method: "POST",
  ...
  setStaffActive: async (username, is_active) => {
    const updated = await apiCall(`/auth/staff/${encodeURIComponent(username)}/active`, {
      method: "POST",
      body: JSON.stringify({ is_active }),
    });
    set((state) => ({
      staff: state.staff.map((s) => (s.username === username ? { ...s, ...updated } : s)),
    }));
    return updated;
  },

  updateStaffPermissions: async (username, permissions) => {
    const updated = await apiCall(`/auth/staff/${encodeURIComponent(username)}/permissions`, {
      method: "POST",
